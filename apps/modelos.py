...
class RBFLayer(Layer):
    ...
    def __init__(self, units, gamma, **kwargs):
        ...
        self.units = units
        #self.gamma = K.cast(gamma, K.floatx())
        self.gamma = tf.cast(gamma, tf.float32)
    def build(self, input_shape):
        self.mu = self.add_weight(name='mu',
                                  shape=(self.units, int(input_shape[1])),  # Cambié el orden
                                  # shape=(int(input_shape[1]), self.units),
                                  initializer='uniform',
                                  trainable=True)
        print(f"Forma de self.mu en build: {self.mu.shape}")  # 🔍 Depuración
        super(RBFLayer, self).build(input_shape)

    def call(self, inputs):
        print(f"Forma de inputs en RBFLayer: {inputs.shape}")  # Depuracion
        print(f"Forma de self.mu en RBFLayer: {self.mu.shape}")  # Depuración
	#diff = K.expand_dims(inputs) - self.mu
        #diff = tf.expand_dims(inputs) - self.mu
...
def predecir(autor1,autor2, G, G_sub, data_test, modelos):
    if autor1 not in G_sub.neighbors(autor2):
        ...
        print(f"Forma de prueba antes de predecir: {prueba.shape}")
